import socket
import threading
from des_core import DES, pad_text, unpad_text, generate_random_key
from rsa_key_exchange import RSA, serialize_public_key
...
class DESCentralServer:
    ...
    def start(self):
        ...
        print("\n[SERVER] Generating RSA keys for secure key exchange...")
        self.public_key, self.private_key = self.rsa.generate_keypair()
        
        # Generate DES shared key
        self.shared_key = generate_random_key()
        print(f"\n[DES KEY] Generated shared DES key: {self.shared_key}")
        print("[INFO] This key will be encrypted with RSA before sending to clients")
    ...
    def accept_connections(self):
        ...
        while self.running and device_number <= 2:
            try:
                ...
                public_key_str = serialize_public_key(self.public_key)
                client_socket.send(f"RSA_PUBLIC_KEY:{public_key_str}".encode('utf-8'))
                print(f"[RSA] Sent public key to {device_info['name']}")
                
                # Wait for client's RSA public key
                try:
                    client_rsa_data = client_socket.recv(4096).decode('utf-8')
                    if client_rsa_data.startswith("CLIENT_RSA_PUBLIC_KEY:"):
                        client_public_key_str = client_rsa_data.split(":", 1)[1]
                        from rsa_key_exchange import deserialize_public_key
                        device_info['rsa_public_key'] = deserialize_public_key(client_public_key_str)
                        print(f"[RSA] Received public key from {device_info['name']}")
                except Exception as e:
                    print(f"[ERROR] Failed to receive RSA key from {device_info['name']}: {e}")
                ...
                if device_number > 2:
                    print("\n[INFO] Both devices connected!")
                    print("[INFO] Distributing DES key using RSA encryption...")
                    print("="*70)
                    
                    # Send the encrypted DES key to both devices
                    with self.device_lock:
                        for device in self.devices:
                            try:
                                # Encrypt DES key with client's RSA public key
                                encrypted_des_key = self.rsa.encrypt_des_key(
                                    self.shared_key, 
                                    device['rsa_public_key']
                                )
                                
                                # Send encrypted key
                                device['socket'].send(f"ENCRYPTED_DES_KEY:{encrypted_des_key}".encode('utf-8'))
                                print(f"[RSA] Sent encrypted DES key to {device['name']}")
                                
                            except Exception as e:
                                print(f"[ERROR] Failed to send key to {device['name']}: {e}")
                    
                    print("\n[SUCCESS] Secure key exchange complete!")
                    print("[INFO] Chat is now active with end-to-end DES encryption")
                    print("="*70)
